...
class UserManager(BaseUserManager):
    def create_user(self, user_id, password, email, hp, name, sex, age, auth, **extra_fields):
        user = self.model(
            user_id = user_id,
            email = email,
            hp = hp,
    ...
    def create_superuser(self, user_id, password, email=None, hp=None, name=None, sex=None, age=None, auth=None):
        user = self.create_user(user_id, password, email, hp, name, sex, age, auth)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    objects = UserManager()
    address = models.CharField(max_length=128, verbose_name="주소",null=True, default='')
    ...
    sex = models.CharField(choices=GENDER_CHOICES, max_length=8, verbose_name="성별", null=True)
    age = models.CharField(choices=AGE_CHOICES, max_length=18, verbose_name="나이", null=True)
